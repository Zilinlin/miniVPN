...
int main(int argc, char *argv[]){
  ...
  unsigned char * packet = (unsigned char *)malloc(500+ hmac_len);
  int final_len = 0;
  ...
  while(1) {
    ...
    if(FD_ISSET(tap_fd, &rd_set)){
      ...
      do_debug("kkkkkkk1");
      //HMAC(EVP_sha256(), key, strlen(key), ciphertext, strlen(ciphertext), hmac, &hmac_len);
      //unsigned char * packet = (unsigned char *)malloc(strlen(ciphertext)+ hmac_len);
      ...
      if(cliserv==CLIENT){
          ...
          nwrite = sendto(net_fd, ciphertext, ciphertext_len, 0, (struct sockaddr *)&local, sizeof(local));
          if(nwrite < 0){
		        perror("sendto");
		        exit(EXIT_FAILURE);
          }
    ...
    //memcpy(rec_data, rec_packet, nread-hmac_len);
    //unsigned char *rec_hmac = (unsigned char*)malloc(hmac_len);
    //memcpy(rec_hmac, rec_packet+nread-hmac_len, hmac_len);
    ...
    //  do_debug("HMAC-SHA256 verification succeeded");
    //}
    // (char *)&plength, sizeof(plength), 
    unsigned char *decrypted_text = (unsigned char *)malloc(nread + EVP_CIPHER_CTX_block_size(dec_ctx));
    int decrypted_len = 0;
    if (!EVP_DecryptUpdate(dec_ctx, decrypted_text, &decrypted_len, rec_packet, nread)) {
